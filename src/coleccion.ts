...
*/
	...
	/**
	 * Elimina una carta de la colección con promesas.
	 * @param id El ID de la carta a eliminar.
	 * @param usuario El nombre del usuario.
	 */
	public eliminarCarta(id: number,usuario: string): Promise<string> {
		const DirectorioUsuario = `./cartas/${usuario}`;
		const RutaCarta = `${DirectorioUsuario}/${id}.json`;
		
		return new Promise((resolve, reject) => {
			fs.unlink(RutaCarta, (error) => {
				if (error) {
					reject('Error al eliminar la carta.');
					return;
				}
				resolve('Carta eliminada correctamente.');
			});
		});
	}
...
*/
	...
	/**
	 * Muestra la información de una carta específica en la colección de un usuario con promesas.
	 * @param id El ID de la carta a mostrar.
	 * @param usuario El nombre del usuario.
	 */
	public mostrarCarta(id: number, usuario: string): Promise<string> {
		const DirectorioUsuario = `./cartas/${usuario}`;
		const RutaCarta = `${DirectorioUsuario}/${id}.json`;

		return new Promise((resolve, reject) => {
			fs.readFile(RutaCarta, 'utf8', (error, data) => {
				if (error) {
					reject('Error al buscar la carta.');
					return;
				}
				this.ImprimirConColor(data);
				resolve(`Carta encontrada!`);
			});
		});
	}
